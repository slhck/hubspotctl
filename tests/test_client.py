...
def _mock_response(client: HubSpotClient, status: int = 200, json: Any = None) -> MagicMock:
    resp = MagicMock()
    resp.status_code = status
    resp.json.return_value = json
    client._client.request.return_value = resp  # type: ignore[union-attr]
    return resp
...
class TestHubSpotClient:
    ...
    def test_delete_returns_none(self, client: HubSpotClient) -> None:
        ...
        result = client.delete_contact("101")
    ...
    def test_get_contact_by_email_uses_id_property(self, client: HubSpotClient) -> None:
        ...
        call_kwargs = client._client.request.call_args  # type: ignore[union-attr]
        assert "idProperty" in str(call_kwargs)

    def test_get_deal_pipelines_extracts_results(self, client: HubSpotClient, mock_pipelines: list[dict]) -> None:
    ...
    def test_list_owners_extracts_results(self, client: HubSpotClient, mock_owners: list[dict]) -> None:
    ...
    def test_crud_methods_callable(self, client: HubSpotClient, method: str, args: list) -> None:
        _mock_response(client, json={"results": [], "total": 0})
        getattr(client, method)(*args)
        client._client.request.assert_called_once()  # type: ignore[union-attr]
