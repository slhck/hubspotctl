...
@click.option(
    ...
    "filters",
    multiple=True,
    help="Filter by property (e.g. -F last_touchpoint='MWC 2026'). Operators: = != < > <= >= ~",
)
@pass_context
def list_contacts(
    ctx: Context,
...
@contact.command("merge")
@click.argument("primary_id")
@click.argument("merge_id")
@click.confirmation_option(
    prompt="Merge these contacts? The second will be merged into the first and archived."
)
@pass_context
def merge_contacts(ctx: Context, primary_id: str, merge_id: str) -> None:
    """Merge two contacts into one.

    PRIMARY_ID is the contact to keep; MERGE_ID is merged into it and then
    archived. Both must be numeric record IDs, not email addresses.
    """
    client = ctx.ensure_client()
    merge_records(client, "contacts", primary_id, merge_id)


@contact.command("associate")
@click.argument("contact_id")
@click.option(
    "--company",
    "-c",
...
@click.option(
    ...
    "labels",
    multiple=True,
    help="Association label name or type ID (repeatable). See 'contact labels'.",
)
@pass_context
def associate_contact(
    ...
    """Associate a contact with companies and/or deals.

    CONTACT_ID must be the numeric record ID, not an email address.
    """
    client = ctx.ensure_client()
    change_associations(
        client,
...
@pass_context
def disassociate_contact(
    ctx: Context,
    contact_id: str,
    companies: tuple[str, ...],
    deals: tuple[str, ...],
) -> None:
    """Remove associations between a contact and companies and/or deals.
...
@contact.command("associations")
@click.argument("contact_id")
@pass_context
def contact_associations(ctx: Context, contact_id: str) -> None:
    """List companies and deals associated with a contact.

    CONTACT_ID must be the numeric record ID, not an email address.
    """
    client = ctx.ensure_client()
    show_associations(
        client, "contacts", contact_id, ["companies", "deals"], ctx.format
    )


@contact.command("labels")
@pass_context
def contact_labels(ctx: Context) -> None:
    """List association labels available from contacts to companies and deals."""
    client = ctx.ensure_client()
    show_labels(client, "contacts", ["companies", "deals"], ctx.format)
